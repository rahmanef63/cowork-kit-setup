...
description: >-
  Generate a complete, runnable automation kit for ANY field or industry, tuned for Claude Cowork. From one field name it derives field-specific best practices and scaffolds a project in the projects/ folder, runnable several ways: a drop-in Cowork skill, a local Python CLI (Anthropic and Claude Agent SDK), a local-filesystem Next.js CRUD website, and a bundled MCP server that lets Cowork create, read, update, and delete the website's data — one shared tool contract and datastore. Use whenever someone wants to set up, bootstrap, or generate Cowork or Claude automations for a business, role, or field, or wants an agent project scaffold with tool-calling, a local dashboard, or an MCP CRUD server. Trigger even without the word 'Cowork'. Do NOT use for one-off scripts, fixing an existing app, image or slide generation, or merely explaining Cowork.
